...
module Profiles
  ...
  # The fan-out that rewrites the denormalized author snapshot on everything a
  # user has written, after they change their display name or avatar.
  #
  # A COMMAND, not a module. It used to be deliberately kept out of the domain on
  # the grounds that a Lambda is not automatically an RPC endpoint — true, but it
  # conflated "how is this invoked" with "what is this". Under Foobara those are
  # separate: this is a command like any other, and the only difference from
  # UpdateProfile is which connector it is connected to. It gains input
  # validation, a typed result and a manifest entry by being one.
  class ReconcileAuthorSnapshot < Foobara::Command
    extend Foobara::AWS::Lambda
    # A fan-out over everything one user has written, so the default 10 seconds
    # is the wrong shape entirely — a prolific author would time out, the message
    # would return to the queue, and the work would restart from the beginning
    # each time until the queue gave up on it. Mostly DynamoDB round trips, so
    # time is what it needs rather than memory.
    aws_lambda timeout: 120

    description "Rewrite the author snapshot on this user's posts and comments."
    ...
    # Rewrites to the CURRENT value, so redelivery is harmless — which SQS
    # requires, being at-least-once.
    def execute
      ProfileReconciliation.perform(user_sub)
    end
  end
  ...
  class UpdateProfile < Foobara::Command
    ...
    def execute
      ...
      # Enqueues when a queue is configured, runs inline when one is not — so
      # local and test behaviour is unchanged and the queue stays a deployment
      # concern. Generated by SQSConnector#connect; see config/connector.rb.
      ReconcileAuthorSnapshotAsync.run!(user_sub: viewer.sub)
